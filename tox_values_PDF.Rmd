...
#### All values in units of _ug/m3_.
...
rass <- rass %>%
  ...
  rename(CAS = `CAS # or MPCA#`, 
        ...
        `Acute air reference` = `Tox Value Source..6`,
         `Cancer based air concentration` = `10-5 Cancer-based Air Conc (ug/m3)`,
         `Cancer based air reference` = `Tox Value Source..13`,
         `Non-cancer chronic air concentration` = `Reference Conc. (ug/m3)..20`,
         `Non-cancer chronic air reference` = `Tox Value Source..19`) %>%
  mutate(`Acute air concentration` = round(as.numeric(`Acute air concentration`), digits = 4),
         `Cancer based air concentration` = round(as.numeric(`Cancer based air concentration`), digits = 4),
         `Non-cancer chronic air concentration` = round(as.numeric(`Non-cancer chronic air concentration`), digits = 4)) %>%
  filter(!is.na(CAS)) 
rass <- rass %>%
...
names <- c("Acronym", "Source name", "Web link")
...
iris <- c("IRIS", "USEPA Integrated Risk Information System", "`www.epa.gov/IRIS/`")
...
names(acronym_table) = as.character(unlist(acronym_table[1, ])) # the first row will be the header
